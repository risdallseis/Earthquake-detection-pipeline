# -*- coding: utf-8 -*-
"""
Created on Fri Apr 12 12:51:29 2019
...
#files to xcorr
files2xcor=[sys.argv[5],sys.argv[6],sys.argv[7]]
filesxcor = [sys.argv[14],sys.argv[15],sys.argv[16]]

#station locations MUST BE IN THE SAME ORDER AS Z COMPS
s1=np.array([[-2.88664,53.75959]])#AQ03
s2=np.array([[-2.969,53.776]])#AQ04
s3=np.array([[-2.968,53.814]])#AQ05
s4=np.array([[-2.911,53.825]])#AQ06
...
for filename in files:
	...
	st2 = tr_filt.copy()
	trig = coincidence_trigger("recstalta", 2.5, 1, st2, 4, sta=0.5, lta=2.0)#4 stations must trigger in order to be recorded
...
output2=[item for item in possquakes if item[0] >6]
...
for d in possqtimes:
	...
	for filen, filem in zip(files2xcor, filesxcor):
		st += read(filem,starttime=d-2, endtime=d+3)#xcorr from same station
...
eq_mags_sarg=[]
for i in Tquakes:
	BGS_mag, o_sarg_mag=magnitude(ncomp4,i,2,station2hypo[1])#using station AQ04 to calc mag
	...
	zt1=read(zcomp5,starttime=(UTCDateTime(i))-2, endtime=(UTCDateTime(i))+3)
	tr_filt = zt.copy()[0]
	tr_filt2 = zt1.copy()[0]
	...
	p_pick5, phase_info = pk_baer(st3.data, df,20, 20, 60, 12.0, 100, 100)
	if p_pick5<2:
		p_pick5=random.uniform(station2hypo[2]/0.055,station2hypo[2]/0.045)
	p_arrivals.append(p_pick5/100)
	D1=(p_pick4/100)+((station2hypo[0]-station2hypo[1])/velocity)
	D3=(p_pick4/100)+((station2hypo[3]-station2hypo[1])/velocity)
